...
} from "@/lib/ai-chat/prompt-run/lightweight-audit";
import { AI_CHAT_MEMORY_MODEL } from "@/lib/ai-chat/constants";
import { logAiChat } from "@/lib/ai-chat/observability";
import { parseLlmJsonObject, stripNullFields } from "@/lib/ai-chat/llm-json";
import {
...
export async function extractOnboardingProfile(
...
): Promise<OnboardingExtraction | null> {
  ...
  if (!parsedResult) {
    if (msg.stop_reason === "max_tokens") {
      logAiChat("warn", "onboarding_extractor_max_tokens_unparseable", {
  ...
  if (parsedResult.salvaged) {
    logAiChat("warn", "onboarding_extractor_json_salvaged", {
      stopReason: msg.stop_reason,
      outputTokens: msg.usage?.output_tokens,
    });
  } else if (msg.stop_reason === "max_tokens") {
    logAiChat("warn", "onboarding_extractor_max_tokens", {
      outputTokens: msg.usage?.output_tokens,
    });
  }
  ...
  const out = onboardingExtractionSchema.safeParse(parsed);
  if (!out.success) {
    logAiChat("warn", "onboarding_extractor_schema_mismatch", {
