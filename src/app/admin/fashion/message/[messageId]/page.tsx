...
export default function FashionAdminMessagePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await adminFetch(
        `/api/admin/fashion/messages/${encodeURIComponent(messageId)}`,
        token,
      );
      if (!res.ok) {
        const body = (await res.json().catch(() => null)) as { error?: string } | null;
        throw new Error(body?.error ?? `Run not found (${res.status})`);
      }
  ...
  if (error || !detail) {
    return (
      <div className="card border-red-200 bg-red-50/50 px-6 py-8 text-sm text-red-900">
        {error ?? "Fashion catalog run not found."}
      </div>
    );
  }
